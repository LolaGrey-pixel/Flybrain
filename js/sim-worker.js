...
/* ---------- constants ---------- */
// ИСПРАВЛЕНО: новое имя файла FisHKA-worker
importScripts('fisHKA-worker-v3.js');
...
function buildGroupStructures() {
	...
	/* FisHKA: инициализация */
	if (typeof FisHKA_Worker !== 'undefined') { FisHKA_Worker.init(numGroups); }
}
...
function tick() {
	...
	var fishkaResult = null;
	if (typeof FisHKA_Worker !== 'undefined') {
		fishkaResult = FisHKA_Worker.update(groupSpikeCounts);
	}
	...
	self.postMessage({
		...
		tickCount: tickCount,
		/* ИСПРАВЛЕНО: было fishka, стало fishkaFeatures — совпадает с тем,
		 * что читает bridge в case 'tick' → FisHKA_Main.updateHologram(..., e.data.fishkaFeatures) */
		fishkaFeatures: fishkaResult ? fishkaResult.features : null,
		fishkaPCM: fishkaResult ? fishkaResult.pcm : null
	});
	tickCount++;
